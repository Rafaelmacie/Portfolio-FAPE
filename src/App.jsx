    ...
    versoes: [
      {
        ...
        data: "04/05/2026",
        features: [
          { nome: "Controle de Dados Bancários", status: "Operacional" },
      ...
      },
      {
        ...
        features: [
          { nome: "Controle de Dados Bancários", status: "Operacional" },
          { nome: "Controle de Integralizações", status: "Operacional" },
          { nome: "Gestão de sobras e fundos", status: "Operacional" },
    ...
    versoes: [
      {
        ...
        data: "04/05/2026",
        features: [
          { nome: "Gestão de Acessos e Perfis", status: "Em Desenvolvimento" },
      ...
      },
      {
        ...
        data: "08/06/2026",
        features: [
          { nome: "Gestão de Acessos e Perfis", status: "Operacional" },
          { nome: "Gestão dos dados da cooperativa", status: "Em Desenvolvimento" },
...
const ImageCarousel = ({ images, onExpand, heightClass }) => {
  ...
  return (
    ...
    >
      ...
      {images.length > 1 && (
        <>
          <button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 p-2.5 rounded-full bg-white/90 text-slate-800 shadow-lg backdrop-blur-sm opacity-0 group-hover:opacity-100 hover:bg-emerald-50 hover:text-emerald-700 transition-all z-10">
            <ChevronLeft size={24} />
          </button>
          <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 p-2.5 rounded-full bg-white/90 text-slate-800 shadow-lg backdrop-blur-sm opacity-0 group-hover:opacity-100 hover:bg-emerald-50 hover:text-emerald-700 transition-all z-10">
            <ChevronRight size={24} />
          </button>
          <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 flex gap-2">
            {images.map((_, idx) => (
              <button key={idx} onClick={(e) => { e.stopPropagation(); setCurrentIndex(idx); }} className={`w-2 h-2 rounded-full transition-all duration-300 ${currentIndex === idx ? 'bg-emerald-500 w-6' : 'bg-slate-300 hover:bg-emerald-300'}`} />
            ))}
          </div>
        </>
      )}
...
export default function PortfolioFape() {
  ...
  return (
    <>
      {expandedImage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm p-4 md:p-10 transition-opacity animate-in fade-in" onClick={() => setExpandedImage(null)}>
          <button className="absolute top-6 right-6 p-2 rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors backdrop-blur-md" onClick={() => setExpandedImage(null)}><X size={24} /></button>
          <img src={expandedImage.src} alt={expandedImage.caption} className="max-w-full max-h-full rounded-xl shadow-2xl object-contain animate-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()} />
          <div className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-black/60 text-white px-6 py-2 rounded-full backdrop-blur-md text-sm font-medium whitespace-nowrap">{expandedImage.caption}</div>
        </div>
      )}
      ...
      <div className="flex h-screen w-full bg-slate-50 text-slate-800 font-sans overflow-hidden selection:bg-emerald-200 selection:text-emerald-900">
        <div className="fixed inset-0 z-0 pointer-events-none flex justify-center"><div className="w-[80vw] h-[80vw] bg-emerald-50/50 rounded-full blur-[120px] -translate-y-1/2" /></div>
        ...
        <aside className={`${isSidebarCollapsed ? 'w-20' : 'w-64 md:w-72'} flex flex-col bg-white border-r border-slate-200 relative z-20 shrink-0 shadow-sm transition-all duration-300`}>
          ...
          <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto [&::-webkit-scrollbar]:hidden">
            {!isSidebarCollapsed && <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-3 mt-1 px-3 animate-in fade-in whitespace-nowrap">Módulos do Sistema</p>}
            <button onClick={() => scrollToSection('home')} className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center px-0' : 'gap-3 px-4'} py-3 text-sm rounded-lg transition-all duration-300 ${activeSection === 'home' ? 'bg-emerald-50 text-emerald-700 font-bold shadow-sm border border-emerald-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 font-medium'}`}><Home size={20} />{!isSidebarCollapsed && <span className="whitespace-nowrap">Visão Geral</span>}</button>
            {MODULOS_DATA.map((modulo) => (
              <button key={modulo.id} onClick={() => scrollToSection(modulo.id)} className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center px-0' : 'gap-3 px-4'} py-3 text-sm rounded-lg transition-all duration-300 group ${activeSection === modulo.id ? 'bg-emerald-50 text-emerald-700 font-bold shadow-sm border border-emerald-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 font-medium'}`}><span className={`transition-colors duration-300 ${activeSection === modulo.id ? 'text-emerald-600' : 'text-slate-400 group-hover:text-emerald-500'}`}>{modulo.icone}</span>{!isSidebarCollapsed && <span className="whitespace-nowrap">{modulo.titulo}</span>}</button>
            ))}
          </nav>

          <div className="p-4 border-t border-slate-100 flex justify-center">
            <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="p-2 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors w-full flex justify-center">{isSidebarCollapsed ? <PanelLeftOpen size={20} /> : <PanelLeftClose size={20} />}</button>
          </div>
        </aside>

        <main className="flex-1 w-full h-full overflow-y-auto scroll-smooth relative z-10 [&::-webkit-scrollbar]:hidden">
          <div className="w-full pb-32">
            <section id="home" className="min-h-[85vh] w-full flex flex-col xl:flex-row items-center justify-center py-16 px-8 md:px-16 lg:px-24 gap-12 lg:gap-16">
              <div className="flex-1 w-full flex flex-col items-start text-left">
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-900 mb-6 leading-tight tracking-tight w-full">Cooperativas<span className="text-emerald-600">CE</span></h2>
                <h3 className="text-4xl md:text-5l lg:text-6l font-extrabold text-slate-900 mb-6 leading-tight tracking-tight w-full">O sistema do futuro <span className="text-emerald-600">cooperativista.</span></h3>
                <p className="text-base md:text-lg text-slate-600 leading-relaxed w-full">Explore a documentação viva do Sistema de Gerenciamento para Cooperativas. Navegue pelos painéis para entender a estrutura, o design e o progresso das funcionalidades desenvolvidas.</p>
              </div>
              <div className="flex-1 w-full xl:max-w-xl relative mt-8 xl:mt-0">
                <ImageCarousel images={CAROUSEL_IMAGES_HOME} onExpand={setExpandedImage} heightClass="h-[300px] md:h-[400px]" />
              </div>
            </section>
